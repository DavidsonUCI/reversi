...
io.sockets.on('connection', function(socket){
	...
	*}
		join_room_response:
		{
			...
			'membership' : number of people in the room including the new One_Room
		}
		or
		{
			'result' : 'fail',
	...
	*/
	...
	socket.on('join_room',function(payload){
		...
		if(('undefined' === typeof payload) || !payload){
			...
			/* when a web page disconnects alert everyone in the board */
...
socket.on('send_message', function(payload){
	...
	if(('undefined' === typeof payload) || !payload){
		...
		/* payload:
		 {
				'requested_user' : user name of person sending the message,
			}
			invite_response:
			{
				'result' : 'success',
		...
		*/
		...
		socket.on('invite', function(payload){
			...
			if(('undefined' === typeof payload) || !payload){
				var error_message = 'invited no payload, command aborted';
				log(error_message);
				...
				/* Check that the message can be traced to a username */
					...
					if(('undefined' === typeof requested_user) || !requested_user){
						var error_message = 'invite didn\'t specify a requested, command aborted';
						log(error_message);
						socket.emit('invite_response', {
																								result: 'fail',
																								message: error_message
		...
		*/
		...
		socket.on('uninvite', function(payload){
			...
			/* Check to make sure that a payload was sent */
			if(('undefined' === typeof payload) || !payload){
				var error_message = 'uninvited no payload, command aborted';
				log(error_message);
				...
				/* Check that the message can be traced to a username */
					...
					var requested_user = payload.requested_user;
					if(('undefined' === typeof requested_user) || !requested_user){
						var error_message = 'uninvite didn\'t specify a requested, command aborted';
						log(error_message);
						socket.emit('uninvite_response', {
																								result: 'fail',
																								message: error_message
						...
						*/
						...
						socket.on('play_token', function(payload){
							...
							if(('undefined' === typeof payload) || !payload){
								...
								/* Check that the has previously registered */
									var player = players[socket.id];
									if(('undefined' === typeof player) || !player){
									var error_message = 'server doesn\'t recognize you (try going back on screen)';
									log(error_message);
									socket.emit('play_token_response', {
																												result: 'fail',
																												message: error_message
									...
									if(('undefined' === typeof column) || column < 0 || colum  > 7){
									var error_message = 'play_token didn\'t specify a valid row, command aborted)';
									log(error_message);
									socket.emit('play_token_response', {
																												result: 'fail',
...
function send_game_update(socket, game_id, message){
	...
	var numClients;
	do{
		roomObject = io.sockets.adapter.room[game_id];
		numClient = roomObject.length;
		if(numClients > 2){
